...
- `user_notifications` 회사명·테마 보강(v47) — ticker 구독 알림 title 에 `stock_universe.asset_name` 으로 회사명 보강(`'에코프로비엠 (112290)'`), 등장 테마명을 detail 라인에 가운뎃점(·) 구분으로 노출 (다수 테마 시 타이틀에 `(N개 테마)`). 마이그레이션은 `_format_ticker_notification` 단일 포맷터를 import 해 신규/backfill 동일 포맷 보장. UI 변경 없음 (`notifications.html` 의 detail 분기 재사용). 테마 구독 알림은 변경 없음.
- `user_oauth_accounts`(v51) — OAuth(Google + Kakao) provider 계정 연결. PK `id`, FK `user_id` CASCADE, UNIQUE `(provider, provider_user_id)` + `(user_id, provider)`. 한 user 가 Google + Kakao + local 동시 보유 가능 (1:N). Authlib + Starlette SessionMiddleware (state CSRF) 사용. 라우트 `/auth/oauth/{google|kakao}/{start|callback|link|unlink}`. 자동 연결 정책: provider 이메일이 `email_verified=true` 이고 동일 이메일 local 계정 존재 시 자동 연결, 그 외엔 신규 가입. 신규 가입 시 `role='user' / tier='free' / password_hash=NULL` 하드코딩 (권한 상승 차단). 운영 노출은 Cloudflare Tunnel + HTTPS 필수 (Google/Kakao 양쪽 raw IP 거부 정책). `OAUTH_ENABLED=false`(기본) 시 라우트 미등록 + UI 버튼 자동 숨김 → 회귀 0. 운영 세팅: [`_docs/20260525150854_oauth-setup.md`](_docs/20260525150854_oauth-setup.md), 설계 spec: [`_docs/20260525144745_oauth-google-kakao-design.md`](_docs/20260525144745_oauth-google-kakao-design.md).
...
- ticker 구독 알림은 `_format_ticker_notification(sub_key, asset_name, themes)` 단일 포맷터로 생성 — `stock_universe.asset_name` 폴백 시 티커만 표시, 테마 다수 시 타이틀에 `(N개 테마)` + detail 에 `·` 구분. 신규(`shared/db/session_repo.py:_generate_notifications`) / backfill(v47 마이그레이션) 양쪽이 동일 포맷터 import.
