...
### 3.2 신규 테이블 — `stock_universe_fundamentals` (v36)
...
### 4.2 `screener_presets` 스키마 확장 (v37)
...
| 단계 | 변경 | PR 단위 | 예상 |
|---|---|---|---|
| **M1: DB 인프라** | v36 `stock_universe_fundamentals`, v37 `screener_presets` 확장 | 마이그레이션 단독 | 0.5주 |
...
| **M4: 거장 시드** | `analyzer/investor_strategies.py` 8종, v38 마이그레이션 UPSERT, `/api/screener/strategies`, `/presets/{key}/clone` | 백엔드 | 0.5주 |
...
- M2~M3는 신규 컬럼/테이블만 — drop 불필요. 비활성화 시 `FUNDAMENTALS_SYNC_ENABLED=false`.
- M4 시드 문제 발견 시 v39에서 `is_seed=TRUE` row만 DELETE (사용자 복제본 무영향).
