...
![차트: 12-1 모멘텀 — 상위 10% vs 하위 10% 6개월 누적 수익률](/static/edu/charts/momentum-investing-1.svg)

*Jegadeesh-Titman (1993) 효과 — 과거 강한 모멘텀이 미래 6개월에도 지속*
...
| 신호 값 | 조건 (연속일수 기준) | 의미 |
|------|----------------|------|
| `strong_buy` | 5일 이상 연속 순매수 | 강한 추세 진입 |
| `buy` | 3~4일 연속 순매수 | 관심 유효 |
| `neutral` | -4 ~ 2일 (혼조) | 관망 |
| `sell` | 5일 이상 연속 순매도 | 추세 약화 경고 |
...
|----------|------|--------------|
| <5% | 정상 ~ 약한 베팅 | low |
| 5~10% | 의미 있는 베팅 | medium |
| ≥10% | 극단적 베팅 (스퀴즈 위험 큼) | high |

단, 잔고 비율이 5~10%(`medium`)이면서 최근 5일간 잔고가 0.5%p 이상 감소한 경우 `high`로 자동 상향됩니다 (숏커버링 진행 신호).
...
![차트: RSI 다이버전스 — 가격 신고가, RSI 하락](/static/edu/charts/chart-key-five-1.svg)

*약세 다이버전스 — 가격이 신고가를 갱신해도 RSI가 따라오지 못하면 추세 약화 신호*
...
![차트: 볼린저밴드 스퀴즈 후 변동성 폭발](/static/edu/charts/chart-key-five-2.svg)

*변동성 수축(스퀴즈) 후 폭발은 가장 신뢰도 높은 패턴*
