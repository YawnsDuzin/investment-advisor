...
from api.chat_engine import build_theme_context, query_theme_chat_sync, query_theme_chat_stream
...
def create_chat_session(body: CreateSessionRequest, conn=Depends(get_db_conn), user: Optional[UserInDB] = Depends(get_current_user_required)):
...
@router.get("/sessions")
def list_chat_sessions(conn=Depends(get_db_conn), theme_id: int | None = None, user: Optional[UserInDB] = Depends(get_current_user_required)):
...
def get_chat_session(session_id: int, conn=Depends(get_db_conn), user: Optional[UserInDB] = Depends(get_current_user_required)):
...
def delete_chat_session(session_id: int, conn=Depends(get_db_conn), user: Optional[UserInDB] = Depends(get_current_user_required)):
...
async def send_message(
    ...
    응답 본문은 SSE (`/api/chat-stream/theme/{session_id}`) 로 전달.
    """
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        # 1) 세션 + 테마 정보 조회
        cur.execute("""
