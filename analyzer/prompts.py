...
"펀더멘털 시계열"이 제공된 경우, **PER/PBR latest 와 12M 분포 percentile 을 그대로 인용**하여
역사적 고/저평가 구간을 명시하세요 (예: "현재 PER 12 — 12M 분포 상위 8% 의 저평가 구간"). EPS YoY 는 성장 신호 해석에 사용.
...
대시보드 hero 카드용 — **80자 이내** 단문. 핵심 흐름 + 사용자가 챙겨야 할 한 가지.
예: "반도체 메모리 야간 강세, 한국 KOSPI 시초 갭상승 가능성 — 외국인 매수 추이 확인."
모호한 "기대감"·"전망" 금지. 실측 흐름 + 행동 단서.

## 출력 형식

```json
{{
  "one_liner": "80자 이내 한 줄 요약",
  "us_summary": {{
