...
def _run_analysis(cfg: AppConfig, today: str, run_id: int | None, log,
    ...
    try:
        from analyzer.signals import (
            detect_signals,
            detect_foreign_signals,
            generate_watchlist_notifications,
        )
        sig_counts = detect_signals(cfg.db)
        if sig_counts:
            log.info(f"[시그널] OHLCV 시그널 {sum(sig_counts.values())}건 탐지 — {sig_counts}")
        # 외국인 수급 시그널 (KR 한정 — foreign_flow 미수집 시 silent fallback)
        fsig_counts = detect_foreign_signals(cfg.db)
        if fsig_counts:
            log.info(f"[시그널] 외국인 시그널 {sum(fsig_counts.values())}건 탐지 — {fsig_counts}")
        # UI-5: 워치리스트 + ticker 구독 매칭 알림 생성
        notif_count = generate_watchlist_notifications(cfg.db)
        if notif_count:
            log.info(f"[시그널] 워치리스트/구독 알림 {notif_count}건 생성")
    except Exception as e:
